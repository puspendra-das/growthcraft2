...
import { LayoutDashboard, Users, User, HelpCircle, LogOut, Menu, Briefcase, FileText, Search, ClipboardList } from "lucide-react";
...
const sidebarLinks = [
  ...
  { label: "Job Postings", path: "/employer/jobs", icon: ClipboardList },
  { label: "Applications", path: "/employer/applications", icon: FileText },
  { label: "Profile", path: "/employer/profile", icon: User },
  { label: "Support", path: "/employer/support", icon: HelpCircle },
];
    ...
    <div className="min-h-screen bg-muted/30 flex">
      ...
      <aside className={cn(
        ...
        <div className="h-16 flex items-center gap-2 px-5 border-b border-border">
          ...
          <span className="font-bold text-foreground">Employer Portal</span>
        </div>
      ...
      <div className="flex-1 lg:ml-64">
        <header className="sticky top-0 z-30 h-14 bg-background/95 backdrop-blur-md border-b border-border flex items-center px-4 lg:px-6">
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm text-muted-foreground hidden sm:inline">Employer User</span>
            <div className="h-8 w-8 rounded-full bg-primary text-primary-foreground flex items-center justify-center text-xs font-bold">E</div>
          </div>
        </header>
